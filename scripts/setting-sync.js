...
/* 
	Detect if any pending operation exists for a given id (lock/soft/push).
	- Useful to avoid resnapping while a definitive change is queued.
*/
function _bbmmHasPendingFor(id) {
	try {
		return _bbmmPendingOps.some(op => op?.id === id);
	} catch {
		return false;
	}
}
