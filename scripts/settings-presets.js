...
import { hlp_esc, hlp_timestampStr, hlp_saveJSONFile, hlp_pickLocalJSONFile, hlp_normalizePresetName, getSkipMap, isExcludedWith, hlp_injectHeaderHelpButton, toPreview, toPretty, copyPlainText } from "./helpers.js";
...
Hooks.once("ready", async () => {
    ...
    mod.api.openPresetInspector = openPresetInspector;
...
/* ==========================================================================
	Settings Preset Inspector (moved from macros.js)
========================================================================== */
...
async function getAllSettingsPresets() {
	...
	try {
		...
		if (res.status === 404) {
			DL("settings-presets.js | getAllSettingsPresets(): settings-presets.json not found (404), returning empty object");
			return {};
		}

		if (!res.ok) {
			DL(2, "settings-presets.js | getAllSettingsPresets(): fetch failed", { url, status: res.status, statusText: res.statusText });
			return {};
		}
		...
		DL(2, "settings-presets.js | getAllSettingsPresets(): fetch threw, returning empty object", { url, err });
		return {};
	}
}
...
class BBMMPresetInspector extends foundry.applications.api.ApplicationV2 {
	...
	_prepareRows() {
		...
		const rows = this.itemsAll.map(e => ({
			...
			key: e.key ?? "",
			scope: e.scope ?? "",
			visible: e.config ? "visible" : "hidden",
	...
	async _replaceHTML(result, _options) {
		...
		try {
			...
			DL("settings-presets.js | BBMMPresetInspector: applied size clamps.");
		} catch (e) {
			DL(2, "settings-presets.js | BBMMPresetInspector: failed to apply size clamps", e);
		}
	...
	try {
		...
		const onRender = (app) => {
			...
			try {
				...
				DL("settings-presets.js | BBMM Preset Picker: applied size clamps.");
			} catch (e) {
				DL(2, "settings-presets.js | BBMM Preset Picker: failed to apply size clamps", e);
			}
		...
		DL(3, "settings-presets.js | openPresetInspector(): error", err);
